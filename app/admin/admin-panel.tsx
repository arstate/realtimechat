...
export default function AdminPage({ initialPreviewId }: { initialPreviewId?: string } = {}) {
  ...
  const [adminView, setAdminView] = useState<'chat' | 'avatars' | 'users' | 'videotron'>('chat');
  const [userAvatars, setUserAvatars] = useState<{id: string, url: string}[]>([]);
  const [isUploadingAvatar, setIsUploadingAvatar] = useState(false);
  const [allUsers, setAllUsers] = useState<any[]>([]);
  const [userSearch, setUserSearch] = useState('');
  const [videotronViews, setVideotronViews] = useState<{id: string, name: string, config?: any}[]>([]);
  const [activeVideotronViewId, setActiveVideotronViewId] = useState<string | null>(initialPreviewId || null);
  const [showCreateViewModal, setShowCreateViewModal] = useState<boolean>(false);
                    ...
                    {videotronQrSize > 90 && (
                      <span 
                        ...
                        style={{
                          ...
                          transform: `translateY(${videotronQrTextYOffset}px)`
                        }}
                      >
                        SCAN QR
                      </span>
